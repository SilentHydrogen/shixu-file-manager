...
- 每个当前分类（包括预设分类和用户自定义分类）都有一条独立的 `category_lexicons` 词库记录。词库由分类名称、分类描述、关联规则名称/标题关键词/正文关键词/格式和预设同义词组成；规则或分类描述变化时按指纹重建，删除的分类不再参与打分。
- 文件导入时先提取可用正文：纯文本直接读取，图片走 OCR，Office 文档解析 XML，PDF 读取文本层并在必要时对页面 OCR。分类使用同一份 `contentText`，不会把 PDF/Office 二进制当作正文。
- 本地词库打分同时计算语义相关度和关键词覆盖度：有向量时为 `0.65 × 语义相关度 + 0.35 × 关键词覆盖度`，无向量时退化为关键词覆盖度；文件名命中权重高于正文命中，格式规则仍会将不符合格式的候选置零。界面显示的“识别率”是当前分类作业的运行指标（自动归类或用户确认成功数 / 已分析数），不是经过标注集校准的模型准确率。
...
当前仍是每文件一个向量，尚未实现全文分块向量索引。长文件受提取与 token 长度限制；不确定时应人工确认。识别率目前是线上作业成功率，尚未有带真实标签的准确率、召回率和 F1 评估集，后续需要用用户确认结果建立标注集再校准权重和阈值。后台依赖系统延迟调度，不能承诺持续后台常驻或精确闹钟。
