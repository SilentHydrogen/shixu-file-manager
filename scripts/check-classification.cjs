...
assert.equal(s.metrics().recognitionRate,1,'auto classification contributes to recognition rate');assert.equal(s.metrics().autoRate,1);
...
assert.equal(s.metrics().manualConfirmed,1,'accepted review contributes to recognized count');
